...
$.Window = (function() {
    ...
    var parentCallers = [];
    var minWinData = {long: MINIMIZED_LONG, storage: []};
    var setting = {dock: 'left', dockArea: null, animationSpeed: 400, minWinNarrow: MINIMIZED_NARROW, minWinLong: MINIMIZED_LONG, handleScrollbar: true, showLog: false};
    ...
    function constructor(caller, options) {
        var _this = null;
        var windowId = "window_" + (windowIndex++);
        var minimized = false;
        var maximized = false;
        var selected = false;
        ...
        function initialize(instance) {
            ...
            if ($.browser.msie) {
                container.css({paddingRight: 1, paddingBottom: 1});
            }
            ...
            container.css({left: currPos.left, top: currPos.top});
            ...
            var headerHtml = "<div class='window_header window_header_normal ui-widget-header " + cornerClass + "no-resizable " + options.headerClass + "'>" +
                    iconHtml + "<div class='window_title_text'>" + options.title + "</div>" + "<div class='window_function_bar'></div>" + "</div>";
            ...
            if (options.draggable) {
                container.draggable({cancel: ".no-draggable", start: function() {
                        log("drag start");
            ...
            if (options.resizable) {
                container.resizable({cancel: ".no-resizable", alsoResize: frame, start: function() {
                        log("resize start");
                        if (minimized || maximized) {
                            return false;
                        }
        ...
        function showOverlay() {
            ...
            if (options.showModal) {
                overlay.css({opacity: options.modalOpacity});
            } else {
        ...
        function transferToFixed() {
            ...
            var scrollPos = getBrowserScrollXY();
            container.css({position: "fixed", left: currPos.left - scrollPos.left, top: currPos.top - scrollPos.top});
        }
        function transferToAbsolute() {
            var currPos = container.offset();
            container.css({position: "absolute", left: currPos.left, top: currPos.top});
        }
            ...
            if (options.custBtns != null && typeof options.custBtns == 'object') {
                for (var i = 0; i < options.custBtns.length; i++) {
                    ...
                    if (btnData != null && typeof btnData == 'object') {
                        if (btnData.id != null && btnData.callback != null) {
        ...
        function getFrameHeight(windowHeight) {
            ...
            return windowHeight - 20 - footerHeight - 4;
        }
        ...
        function select() {
            ...
            if (maximized == false) {
                ...
                if (options.selectedHeaderClass) {
                    header.addClass(options.selectedHeaderClass);
                }
            }
        ...
        function maximize(bImmediately, bNoSaveDisplay) {
            ...
            if (caller != null) {
                var cpos = getParentPanelStartPos(caller);
                targetCssStyle = {left: cpos.left, top: cpos.top, width: caller.width(), height: caller.height(), opacity: 1};
            } else {
                targetCssStyle = {left: scrollPos.left, top: scrollPos.top, width: screenWH.width, height: screenWH.height, opacity: 1};
            }
            if (bImmediately) {
                container.css(targetCssStyle);
                adjustHeaderTextPanelWidth();
        ...
        function minimize() {
            ...
            targetCssStyle = {opacity: OPACITY_MINIMIZED};
            container.addClass('no-resizable');
        ...
        function restore() {
            ...
            targetCssStyle = {left: rpos.left, top: rpos.top, width: rwh.w, height: rwh.h, opacity: 1};
            hideContent();
            container.animate(targetCssStyle, setting.animationSpeed, 'swing', function() {
                container.css('z-index', zIndex);
                showContent();
        ...
        return{initialize: initialize, getTargetCssStyle: getTargetCssStyle, getWindowId: function() {
                return windowId;
            }, getCaller: getCaller, getContainer: getContainer, getHeader: getHeader, getFrame: getFrame, getFooter: getFooter, alignCenter: alignCenter, alignHorizontalCenter: alignHorizontalCenter, alignVerticalCenter: alignVerticalCenter, select: select, unselect: unselect, move: move, resize: resize, maximize: maximize, minimize: minimize, restore: restore, close: close, hide: hide, show: show, setTitle: setTitle, setUrl: setUrl, setContent: setContent, setFooterContent: setFooterContent, getTitle: getTitle, getUrl: getUrl, getContent: getContent, getFooterContent: getFooterContent, isMaximized: function() {
                return maximized;
            }, isMinimized: function() {
                return minimized;
            }, isSelected: function() {
                return selected;
            }, setIcon: setIcon, showIcon: showIcon, hideIcon: hideIcon, _decreaseMiniIndex: _decreaseMiniIndex, _adjustMinimizedPos: _adjustMinimizedPos, _setOrgWH: function(wh) {
                orgWh.w = wh.width;
                orgWh.h = wh.height;
            }, _transformTitleText: _transformTitleText, toString: function() {
                return'[Window] id=' + windowId + ', title=' + options.title;
            }};
    }
    return{getInstance: function(caller, options) {
            var instance = constructor(caller, options);
            instance.initialize(instance);
            selectWindow(caller, instance);
            pushWindow(instance);
            if (caller != null) {
                if (caller.get(0)._minWinData == null) {
                    caller.get(0)._minWinData = {long: setting.minWinLong, storage: []};
                }
        ...
        }, getWindow: getWindow, getSelectedWindow: function() {
            for (var i = 0, len = windowStorage.length; i < len; i++) {
                var wnd = windowStorage[i];
                if (wnd.isSelected()) {
                    return wnd;
